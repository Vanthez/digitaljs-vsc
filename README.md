...
* `digitaljs-vsc.examples` – opens a folder with predefined .sv and DigitalJS compatible .json examples ready to interact DigitalJS-VSC functionalities. Warning: executing this command will replace currently opened workspace. Examples will not restore themselves to the original form if overridden.
...
* `digitaljs-VSC.fullOptimization` – enables Yosys optimization of the synthesized circuit ('opt -full'). This might make the circuit differ significantly from its HDL specification. It is disabled by default.
...
* `digitaljs-VSC.simulationEngine` – changes how the synthesized circuit is sumulated. Options: `"WebWorker"` (currently disabled and will not take an effect), `"Synchronous"` (default). The synchronous engine is well tested, but slower in comparison to WebWorker.
...
* All .json examples were copied from [here](https://github.com/tilk/digitaljs/tree/master/examples) and are under `Copyright 2018 Marek Materzok` under BSD 2-Clause. See also `./examples/json/LICENSE`.
* All .vs examples were copied from [here](https://digitaljs.tilk.eu) and are under `Copyright 2018 Marek Materzok` under BSD 2-Clause. See also `./examples/vs/LICENSE`.
* File `src/index.js` which provides DigitalJS API to the webview was copied as a part from [here](https://github.com/tilk/digitaljs/blob/master/examples/template.html) and further modified and is under `Copyright 2018 Marek Materzok` under BSD 2-Clause.
* DigitalJS logo was designed by `Marek Materzok`, is not in this repository, but is displayed under BSD 2-Clause, being sourced onto the webview directly from [here](https://github.com/tilk/digitaljs_online/blob/master/public/android-chrome-192x192.png).
